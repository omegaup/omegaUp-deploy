...
def _main() -> None:
    ...
    parser.add_argument(
        '--overwrite-outs',
        action='store_true',
        help='Overwrite all .out files if a generator is present')
    parser.add_argument(
        '--only-pull-image',
        action='store_true',
    ...
    for p in problems.problems(allProblems=args.all,
        ...
        if args.overwrite_outs:
            logging.info('Removing old .out files: %s...', p.title)
        ...
        problemOutputsDirectory = os.path.join(args.results_directory, p.path,
                                               "outputs")
        ...
        # TODO: add a way of specifying which problems should never have their
        # .out files autogenerated.
        outputsArgs = [
            '-outputs',
